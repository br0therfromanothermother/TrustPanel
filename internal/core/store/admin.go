...
// namespace id equals its username (the 1:1 mapping), and an admin role lands in
// the infra namespace (""). It fails if the username already exists. Adding a
// further member to an existing namespace uses CreateAccountIn.
...
func (s *Store) AppendEvent(ctx context.Context, e model.Event) error {
	if _, err := s.pool.Exec(ctx,
		`INSERT INTO events (kind, severity, message, message_ru, actor, owner_id, op_id) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
		string(e.Kind), string(e.Severity), e.Message, e.MessageRU, e.Actor, e.OwnerID, e.OpID); err != nil {
		return err
	}
...
func (s *Store) ListEvents(ctx context.Context, kind, owner string, limit int) ([]model.Event, error) {
	return s.ListEventsBefore(ctx, kind, owner, limit, time.Time{}, 0)
}

// ListEventsBefore is ListEvents continued: (beforeAt, beforeID) is the position
// of the oldest event already shown, which starts the next page strictly below it.
// The pair is compared as a row value against the same (at, id) the ordering
// uses, since an id alone would be wrong for any event whose timestamp does not
// follow its id (a backfill, a clock step) and would hand back rows the caller
// already has.
// A zero position returns the first page.
func (s *Store) ListEventsBefore(ctx context.Context, kind, owner string, limit int, beforeAt time.Time, beforeID int64) ([]model.Event, error) {
	...
	if beforeID > 0 && !beforeAt.IsZero() {
		...
		conds = append(conds, fmt.Sprintf("(e.at, e.id) < ($%d, $%d)", len(args)-1, len(args)))
	}
	q := eventCols + ` FROM events e WHERE ` +
		strings.Join(conds, " AND ") +
		fmt.Sprintf(" ORDER BY e.at DESC, e.id DESC LIMIT $%d", len(args)+1)
...
// eventCols reads a row plus the size of the operation it belongs to. The size
// counts every namespace, which is not a disclosure: it is the number the
// operation's own summary line states in words, and it lets the panel
// offer to open an operation only when there is something to open.
const eventCols = `SELECT e.id, e.at, e.kind, e.severity, e.message, e.message_ru, e.actor, e.owner_id, e.op_id,
	CASE WHEN e.op_id = '' THEN 0 ELSE (SELECT count(*) FROM events o WHERE o.op_id = e.op_id) END`
...
func scanEvents(rows pgx.Rows) ([]model.Event, error) {
	...
	for rows.Next() {
		var e model.Event
		var kind, sev string
		if err := rows.Scan(&e.ID, &e.At, &kind, &sev, &e.Message, &e.MessageRU, &e.Actor, &e.OwnerID, &e.OpID, &e.OpSize); err != nil {
			return nil, err
		}
...
// ListEventsOfOp returns the lines one operation wrote, oldest first: the order
// they happened in, which is the order they are read in.
//
// It changes nothing about who may read what. owner scopes the rows exactly as
// the journal does; allNamespaces is passed only for the reader who can already
// see every namespace's records, and is the difference between an operation that
// touched three scopes reading as one thing and reading as one line with the
// rest of itself somewhere unreachable. The count of what was left out is
// returned so a reader who cannot see the parts is still told there are parts,
// which the summary line already says in words.
func (s *Store) ListEventsOfOp(ctx context.Context, opID, owner string, crossNamespace bool) ([]model.Event, int, error) {
	if strings.TrimSpace(opID) == "" {
		return nil, 0, nil
	}
	q := eventCols + ` FROM events e WHERE e.op_id = $1`
	args := []any{opID}
	if !crossNamespace {
		q += ` AND e.owner_id = $2`
		args = append(args, owner)
	}
	rows, err := s.pool.Query(ctx, q+` ORDER BY e.at ASC, e.id ASC`, args...)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	// Every row of one operation carries the same total: the first answers
	// how many are not in this list.
	return out, out[0].OpSize - len(out), nil
}
...
// filter (the sentinel "*") would be ambiguous. Callers pass "" for the infra
...
// SetRole changes an account's role in place (admin<->operator). It moves the
// account between no namespaces and touches no owned resource, which is the
// point of decoupling the namespace from the account. Authorization (who may
...
//     members remain, which never strands a namespace without member management;
...
func (s *Store) DeleteAdmin(ctx context.Context, username string) error {
	...
	if isAdmin && ns != "" && nsAdmins <= 1 && nsAccounts > 1 {
		return errors.New("cannot delete the last admin of a scope while members remain")
	}
