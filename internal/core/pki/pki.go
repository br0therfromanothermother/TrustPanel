...
// LeafNotAfter returns the expiry of the first certificate in pemData. Nothing
// in the product renews the management certificates: they are minted once, at
// enrollment, and then simply run out, taking the panel's reach into the server
// with them, so the date is worth reading and reporting.
func LeafNotAfter(pemData []byte) (time.Time, error) {
	cert, err := firstCert(pemData)
	if err != nil {
...
// SignerNotAfter returns the expiry of the certificate in caPEM that issued
// leafPEM. caPEM may hold more than one: during a rotation the trust file is a
// bundle of the outgoing and incoming authorities, and only the one that
// actually signed this leaf decides how long the leaf stays verifiable.
func SignerNotAfter(caPEM, leafPEM []byte) (time.Time, error) {
	leaf, err := firstCert(leafPEM)
	if err != nil {
		return time.Time{}, err
	}
	rest := caPEM
	for {
		...
		if block == nil {
			return time.Time{}, fmt.Errorf("no certificate in the trust file signed %q", leaf.Subject.CommonName)
		}
...
func firstCert(pemData []byte) (*x509.Certificate, error) {
