...
// which the caller (the drill CLI) turns into an alert.
...
func (o VerifyOptions) verifyDB(ctx context.Context, work, dbSQL string, rep *VerifyReport) error {
	...
	// store.MigrateOnce), which sorts chronologically (0001_…, 0009_…), so the
	// greatest name is the newest one, rather than a numeric `version` column,
	// which this schema does not have.
	//
	// The ledger also holds non-numeric entries: the bootstrap seed writes
	// `seed_default_policies_v1`, and that sorts after every `00NN_` name. A
	// plain max(name) therefore reported the seed every week no matter what the
	// snapshot contained, so a backup missing the newest migrations looked
	// exactly like a current one. Only numbered names count here.
	if v, err := psql("trustpanel", "-tAc",
		"SELECT COALESCE(max(name) FILTER (WHERE name ~ '^[0-9]'),'') FROM schema_migrations WHERE to_regclass('schema_migrations') IS NOT NULL"); err == nil {
...
func extractTarGz(path, dir string) error {
	return extractSnapshot(path, dir, "")
}

// extractSnapshot unpacks a snapshot, decrypting it first when the name says it
// is encrypted. identityPath is the node's own age key; empty means the default
// location, which is where Create put it.
func extractSnapshot(path, dir, identityPath string) error {
	var src io.Reader
	f, err := os.Open(path)
	if err != nil {
		return err
	...
	if strings.HasSuffix(path, ageSuffix) {
		...
		id, err := NodeIdentity(identityPath)
		if err != nil {
			return fmt.Errorf("%s is encrypted and the node key could not be read: %w", filepath.Base(path), err)
		}
		cipher, err := io.ReadAll(f)
		if err != nil {
			return err
		}
		plain, err := decryptAgeWith(cipher, id)
		if err != nil {
			return err
		}
