...
// idempotent so a re-run converges instead of failing.
package bootstrap
...
type Store interface {
	...
	// GetSettings/SaveSettings seed the deployment defaults (Settings → Defaults)
	// from install values so the panel's provisioning form is pre-filled.
...
type Options struct {
	...
	Single bool
	// ConnectSubdomain is the login-justifying subdomain the endpoint lives on
	// (default "portal"). Heavy traffic on a login page reads as normal, unlike
...
func Run(ctx context.Context, r Runner, opts Options, log func(string)) (Result, error) {
	...
	// cluster was dropped (e.g. a prior teardown) keeps the package with no
	// server, and the role/database steps below have no socket to connect to. Create a
	...
	for _, b := range []struct{ src, dst string }{
		...
		// bootstrapping from a binary that already lives at the install path, where
	...
	if opts.Single {
		...
		// The control-port firewall runs on every node, including the control
		// plane; the single-node branch above already stages it with the rest.
		tpls := EntryUnitTemplates()
		for _, name := range []string{"trustpanel-firewall.service", "trustpanel-firewall.timer"} {
			files = append(files, fileSpec{l.SystemdDir + "/" + name, string(tpls[name]), 0o644})
		}
	}
	for _, f := range files {
	...
	startUnits := "trustpanel-serve.service trustpanel-agent.service trustpanel-bot.service trustpanel-backup.timer trustpanel-verify-restore.timer trustpanel-firewall.timer"
	if opts.Single {
...
func ensurePKI(ctx context.Context, r Runner, l Layout, opts Options, validity time.Duration, step func(string)) error {
	...
	if r.Exists(l.PKIDir + "/ca.crt") {
		step("certificate authority already present, reusing")
	...
	if cn == "" {
		// The issuer DN travels on every leaf the control channel presents, so it
		// must not name this software. Anchor it to the deployment's own brand,
		// matching the cover identity the rest of the install already wears.
		if b := strings.TrimSpace(opts.Brand); b != "" {
			cn = b + " CA"
		} else {
			cn = "Internal CA"
		}
	...
	step("certificate authority + controller/node certs issued")
	return nil
}
...
func (o Options) populateDB(ctx context.Context, dsn string, dialIn *model.DialIn, res *Result, step func(string)) error {
	...
	// Seed the deployment defaults (Settings → Defaults) from install values so the
	// panel's provisioning form is pre-filled and never re-typed. Only fills blanks,
	// so a re-run or later panel edits are never clobbered.
	if s, err := st.GetSettings(ctx); err == nil {
		changed := false
		...
		setIfEmpty(&s.Fleet.Apex, o.Domain)
		setIfEmpty(&s.Fleet.Brand, o.Brand)
		setIfEmpty(&s.Fleet.ACMEEmail, o.ACMEEmail)
		setIfEmpty(&s.Fleet.RealitySNI, o.RealitySNI)
		setIfEmpty(&s.Fleet.ConnectSubdomain, o.connectSubdomain())
		if changed {
			if err := st.SaveSettings(ctx, s); err != nil {
				return fmt.Errorf("seed deployment defaults: %w", err)
			}
			step("deployment defaults seeded")
		}
	}
...
// DefaultConnectSubdomain is the login-justifying subdomain the endpoint lives
// on when none is configured. A plain, service-neutral name so the camouflage
// site's portal link reads as an ordinary client login, not as what it fronts.
...
// endpoint on the login subdomain (<sub>.<domain>, endpoint = a login page,
	...
	return []model.Domain{
		{
			...
			Purpose: model.PurposeEndpoint, NodeID: nodeID,
		},
		{
			ID: "dom-" + strings.ReplaceAll(domain, ".", "-"), Hostname: domain,
			Purpose: model.PurposeDecoy, NodeID: nodeID,
		},
	}
}
