...
// deploymentEnv records the deployment branding + login subdomain for the
// camouflage origin (fallback). The panel-managed apex/brand defaults used by
// provisioning live in the settings row (Settings → Defaults); this file only
// feeds the local fallback site and is propagated to a standby for continuity.
...
// comes up on promote, following the active control plane.
...
// (which host to watch via TRUSTPANEL_WATCH_TCP, the target label in
...
// alert bot the panel manages). These placeholders are an optional fallback,
...
// serve.env and its token/admins from the panel DB (Bots tab) at runtime. It
...
// trusttunnel, no ACME, since the exit serves Reality and needs no public cert).
//
// This box is the bootstrapped control plane and an HA primary candidate. Its
// agent binds 0.0.0.0:8443 (mTLS) and not localhost: after a failover the
...
// (pg_dump runs against the hot replica, and off-site copies keep being produced
...
// marker with --scheduled. A panel change then applies on both nodes without
// editing systemd.
