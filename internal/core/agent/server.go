...
// FleetCertSource is an optional capability of a StatusSource: a node running
// mTLS reports when its management certificate and the authority behind it run
// out, so the panel can warn while there is still time to reissue. Consulted
// via type assertion so a dev node running without mTLS need not implement it.
type FleetCertSource interface {
	FleetCertStatus() *agentapi.FleetCertStatus
}
...
func (s *Server) handleStatus(w http.ResponseWriter, req *http.Request) {
	...
	if s.status != nil {
		...
		if fs, ok := s.status.(FleetCertSource); ok {
			resp.FleetCert = fs.FleetCertStatus()
		}
	}
