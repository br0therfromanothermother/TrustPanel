...
// it actually protects against a demoted controller regaining
// control.
func (r *Reconciler) apply(ctx context.Context, ds agentapi.DesiredState, skipClockSkew bool) agentapi.ReconcileResult {
	...
	if len(changed) == 0 {
		...
		// Re-cache even when content is unchanged, which keeps the
...
// byte-identical to the cache, where Apply's file-diff short-circuit would
...
// gets a node's data-plane services running again after a reboot, and a silent
// failure there leaves a node dead indefinitely with no trace of why.
