...
func RunAgent(args []string) {
	...
	caFile := fs.String("ca-file", "", "CA bundle (PEM) for verifying the controller")
	...
	if !*devNoMTLS {
		...
		// Read the dates off the very bytes the listener was built from, so what
		// status reports the one the process is using. Nothing here renews them;
		// reporting is the whole mechanism by which anyone finds out in time.
		status.fleetCert = readFleetCertStatus(caPEM, *certFile)
	}
...
type nodeStatus struct {
	...
	fleetCert *agentapi.FleetCertStatus // nil when running without mTLS
}

// FleetCertStatus implements agent.FleetCertSource (any node running mTLS).
func (n *nodeStatus) FleetCertStatus() *agentapi.FleetCertStatus { return n.fleetCert }

// readFleetCertStatus parses the expiry of this node's management certificate
// and of the authority that signed it. A parse failure is not fatal and not
// even logged as an error: the listener is already built from these bytes and
// works, and the only thing lost is the warning.
func readFleetCertStatus(caPEM []byte, certFile string) *agentapi.FleetCertStatus {
	leafPEM, err := os.ReadFile(certFile)
	if err != nil {
		log.Printf("agent: cannot read %s for expiry reporting: %v", certFile, err)
		return nil
	}
	st := &agentapi.FleetCertStatus{}
	...
	if st.NotAfter == nil && st.CANotAfter == nil {
		return nil
	}
	return st
}
...
// yields no entries instead of failing the whole status response.
