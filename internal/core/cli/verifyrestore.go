...
func RunVerifyRestore(args []string) {
	...
	if *scheduled {
		bc := settings.Backup
		if !bc.VerifyOn() {
			log.Printf("verify-restore: drill disabled in panel settings; skipping the scheduled run")
			return
		}
		age, have := markerAge(*marker)
		interval := bc.VerifyInterval()
		if !backupDue(age, have, interval) {
			log.Printf("verify-restore: not due yet (last drill %s ago, interval %s); skipping the scheduled run", age.Round(time.Minute), interval)
			return
		}
	}
