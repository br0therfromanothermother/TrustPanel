...
// stays because during a failover the panel may be down, and root on the box is the
...
func runAddStandby(args []string) {
	...
	sshKey := fs.String("standby-ssh-key", "", "standby SSH private key file (empty: use the SSH agent, e.g. a forwarded agent, which keeps the key off this host)")
