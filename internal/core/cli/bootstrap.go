...
func RunBootstrap(args []string) {
	...
	hardenFirewall := fs.Bool("harden-firewall", false, "hardening: enable ufw (opens ssh + 443 + 80 + agent); the panel limits the agent port by source IP only when ufw is on")
	hardenFail2ban := fs.Bool("harden-fail2ban", false, "hardening: install + enable fail2ban")
	_ = fs.Parse(args)

	if strings.TrimSpace(*publicIP) == "" || strings.TrimSpace(*singboxBin) == "" {
		log.Fatal("bootstrap: --public-ip and --singbox-bin are required")
	}
	if *single {
		// The one box serves the client endpoint itself, so it needs the domain up
		// front (apex + <connect-subdomain>.<apex> both point here).
		if strings.TrimSpace(*domain) == "" {
			log.Fatal("bootstrap: --single requires --domain (the box's own client endpoint)")
		}
		...
		// Multi-node: the exit terminates no domain. --domain is optional here; when
		// given it seeds the deployment apex (Settings → Defaults); otherwise the
		// first provisioned entry establishes it.
...
func summary(opts bootstrap.Options, res bootstrap.Result, adminPass, dbPass string) string {
	...
	if opts.Single {
		...
		fmt.Fprintln(&b, " panel and hand out its client config / QR. Grow later: Servers → Add server.")
	} else {
		fmt.Fprintf(&b, " Exit node:    %s (%s) · Reality SNI %s · pubkey %s\n", opts.NodeName, opts.NodeID, opts.RealitySNI, res.RealityPub)
		apex := opts.Domain
		if strings.TrimSpace(apex) == "" {
			apex = "(set when you add the first entry)"
		}
		fmt.Fprintf(&b, " Brand/apex:   %s / %s\n", opts.Brand, apex)
		fmt.Fprintln(&b, "------------------------------------------------------------------------")
		fmt.Fprintln(&b, " Next: provision an entry node from the panel (Servers → Add server),")
	...
	if h := opts.Hardening; h != nil && h.Enabled {
		...
		fmt.Fprintln(&b)
		if h.Firewall {
			fmt.Fprintln(&b, "               ufw open: ssh + 443 + 80 + 8443 (agent mTLS).")
		}
		fmt.Fprintln(&b, "               VERIFY the new SSH login works in a SECOND session before")
		fmt.Fprintln(&b, "               closing this one.")
	}
