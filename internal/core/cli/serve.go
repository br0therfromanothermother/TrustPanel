...
func RunServe(args []string) {
	...
	caFile := fs.String("ca-file", "", "CA bundle (PEM) for pushing to agents")
	...
	reconcileInterval := fs.Duration("reconcile-interval", 60*time.Second, "background reconcile interval (0 disables)")
	...
	caKeyFile := fs.String("ca-key-file", "", "CA private key (PEM) that enables remote provisioning of nodes")
	...
	// The deployment apex/brand/ACME defaults stamped onto provisioned entries live
	// in panel settings (Settings → Defaults), seeded at bootstrap, not flags.
	...
	certExpiryDays := fs.Int("cert-expiry-warn-days", 30, "warn when a management (mTLS) certificate expires within this many days")
	certExpiryInterval := fs.Duration("cert-expiry-interval", 12*time.Hour, "how often to check management certificate expiry (0 disables)")
	...
	geoCheckInterval := fs.Duration("geo-check-interval", 7*24*time.Hour, "how often to ask the geo rule-set sources what they now offer (0 disables)")
	...
	// Geo databases: the stored source addresses win over the built-in defaults,
	// so a source chosen in the panel survives a restart.
	if ruleSetProv != nil {
		if err := st.EnsureRuleSetSources(ctx, rulesets.DefaultGeoIPBase, rulesets.DefaultGeoSiteBase); err != nil {
			log.Printf("serve: geo sources: %v", err)
		}
		p.SetRuleSets(ruleSetProv)
		if err := p.ApplyRuleSetSources(ctx); err != nil {
			log.Printf("serve: geo sources: %v", err)
		}
	}
	...
	// health) via α, the primary-side sender; each monitor adds its routed leg.
	...
	if *caFile != "" && *caKeyFile != "" && *provSingbox != "" {
		...
		if err != nil {
			...
			log.Printf("remote node provisioning enabled (units from %s; apex/brand from settings)", *provUnitsDir)
		}
	}

	// Nothing reissues the mTLS material the panel and the agents authenticate
	// each other with; it is minted at enrollment and then runs out quietly, so
	// the panel watches its own pair as well as the dates the agents report.
	if !*devNoMTLS {
		p.SetCertExpiryWatch(*caFile, *certFile, *certExpiryDays)
	}
	...
	go p.RunAccessExpiryLoop(sctx, time.Minute)
	go p.RunConfigHealthLoop(sctx, *expiryAlertInterval)
	go p.RunCertExpiryLoop(sctx, *certExpiryInterval)
	...
	go p.RunRuleSetCheckLoop(sctx, *geoCheckInterval)
	go func() {
		<-sctx.Done()
