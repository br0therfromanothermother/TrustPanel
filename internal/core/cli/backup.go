...
//
// Because it normally runs unattended (the daily timer), any failure in either
// the local dump or the off-site push is also raised via the alert bot. A silently
...
func RunBackup(args []string) {
	...
	pkiDir := fs.String("pki-dir", "/etc/trustpanel/pki", "PKI dir to include (CA key etc.)")
	...
	identity := fs.String("identity-file", backup.DefaultIdentityPath,
		"node's own age key, minted on first use; snapshots are encrypted to it (and to the panel's age recipient, when one is set)")
	plaintext := fs.Bool("no-encrypt", false, "write the snapshot unencrypted (it holds the database and the fleet's private keys; local inspection only)")
	...
	// settings: one panel edit governs the active node and the standby. A manual
	// run (no --scheduled) ignores this and always backs up now. If settings could
	// not be loaded we fall through to the defaults instead of skipping, since losing
	// a backup is worse than an extra one.
	if *scheduled {
		bc := settings.Backup
		if !bc.LocalOn() {
			log.Printf("backup: local backup disabled in panel settings; skipping the scheduled run")
			return
		}
		age, have := newestSnapshotAge(*outDir)
		interval := bc.BackupInterval()
		if !backupDue(age, have, interval) {
			log.Printf("backup: not due yet (last snapshot %s ago, interval %s); skipping the scheduled run", age.Round(time.Minute), interval)
			return
		}
	}
	...
	path, err := backup.Create(ctx, backup.Options{
		...
		IdentityPath: idPath, AgeRecipient: settings.Backup.AgeRecipient,
	})
	if err != nil {
	...
	// Off-site delivery is switched on but there is no bot to carry it: either
	// none is configured, or the one named in the settings has no token. Silence
	// here would leave the operator believing copies are leaving the box. This
	// goes out on the same alert path as a failed delivery.
	sender := settings.BackupSenderToken()
	if sender == "" {
		notify(watchdog.Render(watchdog.MsgBackupOffsiteFailed("no bot is configured to send it"), watchdog.DefaultLang))
		log.Printf("backup: off-site skipped: no bot configured to send with (local snapshot still written)")
		return
	}
	if err := backup.DeliverTelegram(ctx, path,
		backup.TelegramTarget{Token: sender, ChatID: settings.Backup.ChatID},
		backup.DeliverOptions{
			AgeRecipient: settings.Backup.AgeRecipient, ChunkBytes: chunk,
			Lang: operatorLang(ctx, st),
		}); err != nil {
		// Off-site is opportunistic for the local snapshot's sake, but a persistent
		// off-site failure means the fleet has no disaster copy. It alerts.
...
// operatorLang is the language the backup chat is read in. There is no request
// and no recipient here to take it from: it is whatever the accounts on this
// installation actually use; an even split or no accounts at all falls back to
// the source language and not by guessing.
func operatorLang(ctx context.Context, st *store.Store) string {
	if st == nil {
	...
	best, bestN := watchdog.DefaultLang, 0
	for _, a := range admins {
		l := strings.TrimSpace(a.Locale)
		if l == "" {
			continue
		}
		count[l]++
...
// snapshot" so a scheduled run proceeds instead of skipping.
