...
func RunProvision(args []string) {
	...
	endpoint := fs.String("endpoint", "", "entry: this node's VPN endpoint hostname (full host)")
	apex := fs.String("apex", "", "entry: deployment apex domain (camouflage site + --host-landing)")
	hostLanding := fs.Bool("host-landing", false, "entry: also serve the apex as a camouflage-only decoy here (the first entry)")
	brand := fs.String("brand", "", "entry: camouflage-site brand name")
	acmeEmail := fs.String("acme-email", "", "entry: ACME contact (default admin@<apex>)")
	...
	if *refreshUnits {
		...
		refreshNodeUnits(*nodeID, pr, *sshHost, *sshUser, *sshPort, *sshKey, *knownHosts,
			...
			*apex, *endpoint, *acmeEmail, *brand, *hostLanding, *acmeStaging)
		return
	}
	if *makeStandby {
	...
	if pr == model.RoleEntry {
		var extraEnv string
		extraEnv, planFiles, entryDomainRows = entryProvisionExtras(*nodeID, *apex, *endpoint,
			*acmeEmail, *brand, *hostLanding, *acmeStaging)
		agentEnv += extraEnv
	}
...
func entryProvisionExtras(nodeID, apex, endpoint, acmeEmail, brand string, hostLanding, staging bool) (agentEnv string, files []provision.PlannedFile, domains []model.Domain) {
	apex = strings.TrimSpace(apex)
	ep := strings.TrimSpace(endpoint)
	var acmeDomains []string
	domains, acmeDomains = cliEntryDomains(nodeID, ep, apex, hostLanding)
	...
	if brand != "" || apex != "" {
		files = append(files, provision.PlannedFile{
			...
			Body: []byte(fmt.Sprintf("TRUSTPANEL_BRAND=%s\nTRUSTPANEL_DOMAIN=%s\nTRUSTPANEL_CONNECT_SUBDOMAIN=%s\n", brand, apex, connectSubFromEndpoint(ep, apex))),
		})
	}
	return b.String(), files, domains
}

// connectSubFromEndpoint recovers the login subdomain the camouflage site should
// advertise from the endpoint host the operator passed (<sub>.<apex>), so the
// portal link matches the host clients actually connect to. Falls back to the
// deployment default when the endpoint is not a subdomain of the apex.
func connectSubFromEndpoint(endpoint, apex string) string {
	endpoint = strings.ToLower(strings.TrimSpace(endpoint))
	apex = strings.ToLower(strings.TrimSpace(apex))
	if endpoint != "" && apex != "" && strings.HasSuffix(endpoint, "."+apex) {
		if sub := strings.TrimSuffix(endpoint, "."+apex); sub != "" {
			return sub
		}
	}
	return bootstrap.DefaultConnectSubdomain
}

// cliEntryDomains mirrors panel.entryDomains: the endpoint hostname is the VPN
// connection endpoint; with --host-landing this node additionally serves the
// apex as a camouflage-only decoy on the same cert. Empty endpoint => no TLS yet.
func cliEntryDomains(nodeID, endpoint, apex string, hostLanding bool) (domains []model.Domain, acmeDomains []string) {
	endpoint = strings.TrimSpace(endpoint)
	if endpoint == "" {
		return nil, nil
	}
	domains = append(domains, model.Domain{ID: domID(endpoint), Hostname: endpoint, Purpose: model.PurposeEndpoint, NodeID: nodeID})
	acmeDomains = []string{endpoint}
	if hostLanding && apex != "" && !strings.EqualFold(apex, endpoint) {
		domains = append(domains, model.Domain{ID: domID(apex), Hostname: apex, Purpose: model.PurposeDecoy, NodeID: nodeID})
...
func refreshNodeUnits(nodeID string, pr model.PublicRole, sshHost, sshUser string, sshPort int,
	...
	apex, endpoint, acmeEmail, brand string, hostLanding, acmeStaging bool) {
	...
	// so rebuild it here or the refresh strips ACME and the agent would
	// stop managing the TLS cert. Require the same endpoint input provision needs.
	if pr == model.RoleEntry && strings.TrimSpace(endpoint) == "" {
		log.Fatal("provision: --refresh-units on an entry needs --endpoint (and --apex/--brand) so agent.env keeps its ACME config")
	}
	binaries := map[string][]byte{}
	...
	if pr == model.RoleEntry {
		var extraEnv string
		extraEnv, planFiles, _ = entryProvisionExtras(nodeID, apex, endpoint,
			acmeEmail, brand, hostLanding, acmeStaging)
		agentEnv += extraEnv
	}
