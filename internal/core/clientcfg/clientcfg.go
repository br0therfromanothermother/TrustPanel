...
func Build(state model.State, userID, entryNodeID string, opts Options, at time.Time) (Config, error) {
	...
	// rejected by ParseDeepLink on import, so emitting one would be a silently broken
	...
	// The QR landing page is hosted on the public-facing apex (decoy) so
...
// entry's public decoy domain (the apex, e.g. example.com) if present,
// otherwise the given fallback (the endpoint/portal hostname).
func landingHostname(state model.State, nodeID, fallback string) string {
	for _, d := range state.Domains {
		if d.NodeID == nodeID && d.Purpose == model.PurposeDecoy && d.Hostname != "" {
			return d.Hostname
		}
	}
	return fallback
...
// clientExclusions derives client-side split-routing globs from the network-wide
// direct policies that apply to the user (the whole network's, or the user's
// group's). The server-side rule remains authoritative; this is a client
// convenience.
func clientExclusions(state model.State, user model.User) []string {
	...
	for _, p := range state.RoutePolicies {
		...
		if p.Tier != model.TierFleet || p.Action != model.ActionDirect {
			continue
		}
...
func entryHostname(state model.State, nodeID string) (string, []string) {
	...
	if first != "" {
		return first, []string{fmt.Sprintf("entry %q has no endpoint domain; using %q", nodeID, first)}
	}
