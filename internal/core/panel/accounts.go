...
func (p *Panel) handleCreateAdmin(w http.ResponseWriter, r *http.Request) {
	...
	// bound now or it could never reach anything, while a panel password is not.
	...
	// Operators have no panel, so the owner binds their Telegram id here at
	// creation; otherwise a fresh operator could never reach the bot.
	if req.TelegramID != 0 {
		tg := req.TelegramID
	...
	p.recordEvent(r.Context(), model.EventAdmin, model.SeverityInfo,
		journal.Line("account %q created/updated", req.Username), actor, "") // infra action
...
func (p *Panel) handleSetRole(w http.ResponseWriter, r *http.Request) {
	...
	if !authz.CanManageMembers(actor, target.Namespace()) {
		writeErr(w, http.StatusForbidden, "you cannot manage members of that scope")
		return
	}
	if target.Role == role {
	...
	// single-account namespace is fine, as there is no one left to strand. (Mirrors
	...
	if target.Role.IsAdmin() && role == model.RoleOperator {
		...
		if n <= 1 {
			...
			if total > 1 {
				writeErr(w, http.StatusBadRequest, "cannot demote the last admin of a scope while members remain")
				return
			}
		}
	}
	...
	p.recordEvent(r.Context(), model.EventAdmin, model.SeverityWarn,
		journal.Line("role of %q set to %s (scope %q)", username, journal.Term(string(role)), journal.Term(nsLabel(target.Namespace()))), me, actor.Namespace())
	writeJSON(w, http.StatusOK, map[string]string{"username": username, "role": string(role)})
}
...
// not infra-only. An empty or zero id clears the binding.
func (p *Panel) handleSetAccountTelegram(w http.ResponseWriter, r *http.Request) {
	...
	p.recordEvent(r.Context(), model.EventAdmin, model.SeverityInfo,
		journal.Line("Telegram binding updated for %q", me), me, p.account(r).Namespace()) // self-service: own namespace
...
func (p *Panel) handleDeleteAdmin(w http.ResponseWriter, r *http.Request) {
	...
	// cookie keeps working until TTL. protected() also rejects deleted accounts.
	p.sessions.RevokeUser(username)
	// Scope to the actor's namespace so the deletion shows up in the journal of
	// whoever performed it (see handleSetRole).
	p.recordEvent(r.Context(), model.EventAdmin, model.SeverityWarn,
		journal.Line("account %q (%s, scope %q) deleted", username, journal.Term(string(target.Role)), journal.Term(nsLabel(target.Namespace()))), me, actor.Namespace())
	writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
}
...
func (p *Panel) handleChangePassword(w http.ResponseWriter, r *http.Request) {
	...
	// Drop this account's other sessions: a changed password should invalidate
	// its logins elsewhere. The caller's current session is kept so the UI stays
	// usable, and other administrators are left alone.
	p.sessions.RevokeUserExcept(me, token)
	p.recordEvent(r.Context(), model.EventAdmin, model.SeverityWarn,
		journal.Line("password changed for %q (other sessions revoked)", me), me, p.account(r).Namespace()) // self-service: own namespace
	writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
}
