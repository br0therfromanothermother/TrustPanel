...
func (p *Panel) handleRouteTest(w http.ResponseWriter, r *http.Request) {
	...
	if addr, err := netip.ParseAddr(target); err == nil {
		...
		if lerr != nil {
			resolveWarn = journal.Line("could not resolve %s: %s — conditions on countries and subnets cannot be checked", target, lerr.Error())
		}
		for _, a := range got {
	...
	if resolveWarn != nil {
		res.Warnings = append([]render.Phrase{render.Say(resolveWarn)}, res.Warnings...)
	}
