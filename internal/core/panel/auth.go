...
// RevokeUserExcept drops the given account's other sessions, keeping the one
// token passed. A changed password should end that person's logins elsewhere,
// their own on their other machines, and nobody else's: revoking fleet-wide
// would sign every other administrator out, mid-task, whenever one of them
// rotated their password. Returns the number of sessions removed.
func (m *SessionManager) RevokeUserExcept(username, keep string) int {
	m.mu.Lock()
	defer m.mu.Unlock()
	n := 0
	for t, s := range m.sessions {
		if t == keep || s.username != username {
			continue
		}
		delete(m.sessions, t)
		n++
