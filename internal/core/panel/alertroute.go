...
// the Telegram fan-out is scoped. An operator is paged about the data plane
...
const (
	// audAdmin: control-plane internals, HA/replication, bot-channel health and
	// backups. Only the global admin alert chat is paged.
	audAdmin audience = iota
	// audBroadcast: data-plane status, a node going down or recovering, an entry's
...
// language, leaving each operator to read the alert in their own UI language.
...
// to render the message in, leaving each recipient to read it in their own UI language.
...
func (p *Panel) audienceChats(ctx context.Context, s model.Settings, aud audience) []chatTarget {
	...
	if aud == audBroadcast {
		// Every account with a bound alert chat, which tells operators that a shared
...
// client config): the owning operator's chat only. The client identity does
