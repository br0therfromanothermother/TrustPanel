...
// (endpoint / connection point), the rest are extra served names; all go
// into the single certificate.
func literalEntryDomains(nodeID string, hosts []string) (domains []model.Domain, acme []string) {
	for i, h := range hosts {
		purpose := model.PurposeDecoy
		if i == 0 {
			purpose = model.PurposeEndpoint
		}
...
func (p *Panel) handleAddEntry(w http.ResponseWriter, r *http.Request) {
	...
	//   variant 2 (reuse): B serves A's existing hostnames, derived from A's
	...
	//     the cert (first = client endpoint / endpoint, rest = extra SANs).
	...
	aID := a.ID
	opID := newOpID() // every group this conversion moves is one operation
	jobID := p.jobs.Start("convert to two-node (add entry "+req.Name+")", func(log func(string)) error {
		...
		p.logEgressMoves(bg, groups, model.SeverityInfo, journal.Line("converted to a two-node deployment"), "system", opID)
		if err != nil {
			return fmt.Errorf("switch egress to A: %w", err)
		}
		log(fmt.Sprintf("egress switched to A for %d group(s)", len(groups)))
...
func (p *Panel) switchEgress(ctx context.Context, ids []string, exitID string) ([]egressMove, error) {
	return egress.Switch(ctx, p.store, ids, exitID)
}

// actorOf names who asked for something, for the log.
func actorOf(p *Panel, r *http.Request) string {
	if actor, _, ok := p.currentUser(r); ok {
		return actor
	}
	return "system"
}

// egressMove, nameOr and exitLabel are the package's own names for what the
// egress package defines, so the call sites here read as they always have while
// the bot works from the same definitions.
type egressMove = egress.Move

func nameOr(name, id string) string { return egress.NameOr(name, id) }

func exitLabel(st model.State, id string) journal.Msg { return egress.Label(st, id) }

// logEgressMoves writes one line per group, in the namespace that owns it, so
// each operator sees its own groups move. It deliberately does not write the
// other namespaces' lines where the person who ran the operation can read them:
// a group name is the thing namespaces exist to keep apart.
//
// What the caller does pass is opID, the name of the one operation all these
// lines and its own summary belong to. Without it, draining a shared exit leaves
// four unrelated rows in four places and no way to ask which went together.
//
// sev separates the two ways a group moves: somebody decided it, which is a
// record, or an exit stopped answering and the panel decided it, which is
// something to look at. Marking both as warnings made the journal one colour.
func (p *Panel) logEgressMoves(ctx context.Context, moves []egressMove, sev model.EventSeverity, reason journal.Msg, actor, opID string) {
	for _, m := range moves {
		p.recordOpEvent(ctx, model.EventAdmin, sev,
