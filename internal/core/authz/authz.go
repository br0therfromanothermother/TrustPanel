...
// through ScopeState and their writes through the Can* gates, which keeps the
// owner_id namespace rule in exactly one place. A new UI section or bot command
...
//     a black box. Other namespaces' clients are removed entirely, and the
//     visible aggregate is computed separately (OthersAggregate) so no names leak.
...
// cross-namespace visibility is a deliberate, buried opt-in. A co-owner never
...
func filterPolicies(in []model.RoutePolicy, owner string) []model.RoutePolicy {
	...
	for _, p := range in {
		if p.Tier == model.TierFleet || p.OwnerID == owner {
			out = append(out, p)
		}
	}
	return out
...
// the non-identifying "N clients in other namespaces" count, leaving the buried
...
// namespace. A new resource is owned by its creator's namespace: callers pass
...
// shared infra, so this follows CanManageInfra and not per-node ownership:
...
// CanWriteDomain gates a domain edit. A domain has no owner of its own, being
