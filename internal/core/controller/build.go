...
func BuildDesiredState(state model.State, compiled render.CompiledNode, layout Layout, extraFiles []agentapi.File, revisionID int64, issuedAt time.Time) (agentapi.DesiredState, error) {
	...
	// Every node also carries the control-port allow-list. It is derived from the
	// current control-plane candidate set and applied by the node's own firewall
	// timer; an empty list leaves the port open (fail-open).
	files = append(files, firewallAllowArtifact(layout.SingBoxDir, ControlPlaneAllowIPs(state)))
