...
type StatusResponse struct {
	...
	// node is not an entry (degrade gracefully, never failing status over it).
	...
	// FleetCert reports the management certificate this agent process is really
	// using: the mTLS leaf it presents to the panel, and the authority that
	// signed it. Nothing renews either; they are minted at enrollment and run
	// out, and when they do the panel loses its reach into the server while
	// clients carry on unaffected. Read once at startup from the same files the
	// live TLS config was built from, so this reports what is in use rather than
	// what happens to be on disk now.
	FleetCert *FleetCertStatus `json:"fleet_cert,omitempty"`
}
...
// FleetCertStatus is when a server's management mTLS material stops working.
type FleetCertStatus struct {
	NotAfter   *time.Time `json:"not_after,omitempty"`    // this server's own certificate
	CANotAfter *time.Time `json:"ca_not_after,omitempty"` // the authority that signed it
}
