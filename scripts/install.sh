...
# binaries from GitHub, with no manual scp. It downloads `trustpanel`, `sing-box`
...
#     | sudo bash -s -- --brand ExampleCDN --reality-sni www.example-cdn-target.com
#
...
#
# Required: --brand. --domain is required for --single (the box's own client
# endpoint) and optional for a multi-node exit (presets the deployment apex).
# --reality-sni is required unless --single (a real
...
# --acme-staging). Node ids are internal and auto-generated, so there's normally
# no reason to pass --node-id. --tag pins a release (default: latest); a fork's
# own release comes from the TRUSTPANEL_REPO env var, not a flag.
#
set -euo pipefail
...
if [[ "$SINGLE" -eq 1 ]]; then
  # The one box serves the client endpoint itself, so it needs the domain now.
  [[ -n "$DOMAIN" ]] || die "--domain is required for --single (the box's own client endpoint)"
else
  # Multi-node exit: terminates no domain. --domain is optional (presets the apex).
...
echo "install: trustpanel $(/usr/local/bin/trustpanel version) installed -> /usr/local/bin/trustpanel"
...
BOOT_ARGS=(bootstrap
  --brand "$BRAND"
  --singbox-bin "$SB_BIN")
[[ -n "$DOMAIN" ]] && BOOT_ARGS+=(--domain "$DOMAIN")
